...
{
    internal class TableWidget<T>
    {
        public List<ColumnDef<T>> ColumnsListForReading
        {
            get
            {
        ...
        public List<T> EntryListForReading
        {
            get
            {
                this.RecacheIfDirty();
        ...
        public TableWidget ( TableDef<T> def, Func<IEnumerable<T>> pawnsGetter, int uiWidth, int uiHeight )
        {
            this.def = def;
            this.entryGetter = pawnsGetter;
            this.SetMinMaxSize( def.minWidth, uiWidth, 0, uiHeight );
            this.SetDirty();
        }

        public void TableOnGUI ( Vector2 position )
        {
            ...
            for ( int j = 0; j < this.cachedEntries.Count; j++ )
            {
                ...
                if ( (float)num4 - this.scrollPosition.y + (float)((int)this.cachedRowHeights[j]) >= 0f && (float)num4 - this.scrollPosition.y <= outRect.height )
                {
                    ...
                    Rect entryRect = new Rect( 0f, (float)num4, viewRect.width, (float)((int)this.cachedRowHeights[j]) );
                    ...
                    for ( int k = 0; k < this.def.columns.Count; k++ )
                    {
                        ...
                        this.def.columns[k].Worker.DoCell( rect3, this.cachedEntries[j], this );
                        num2 += num5;
                    }
                    this.ExtraDrawOnEntry( entryRect, this.cachedEntries[j] );
                }
        ...
        protected virtual void ExtraDrawOnEntry ( Rect rect, T entry )
        {
        }
        ...
        private void RecacheEntries ()
        {
            this.cachedEntries.Clear();
            this.cachedEntries.AddRange( this.entryGetter() );
            if ( this.sortByColumn != null )
            {
                if ( this.sortDescending )
                {
                    this.cachedEntries.SortStable( new Func<T, T, int>( this.sortByColumn.Worker.Compare ) );
                }
                else
                {
                    this.cachedEntries.SortStable( ( T a, T b ) => this.sortByColumn.Worker.Compare( b, a ) );
                }
            }
            this.cachedEntries = this.PrimarySortFunction( this.cachedEntries ).ToList<T>();
        }
        ...
        private void RecacheRowHeights ()
        {
            ...
            {
                this.cachedRowHeights.Add( this.CalculateRowHeight( this.cachedEntries[i] ) );
            }
        }
        ...
        private void RecacheSize ()
        {
            ...
            num2 = Mathf.Min( num2, (float)Verse.UI.screenWidth );
            num3 = Mathf.Min( num3, (float)Verse.UI.screenHeight );
        ...
        private float CalculateRowHeight ( T pawn )
        {
            float num = 0f;
            for ( int i = 0; i < this.def.columns.Count; i++ )
            {
        ...
        private float CalculateTotalRequiredHeight ()
        {
            ...
            for ( int i = 0; i < this.cachedEntries.Count; i++ )
            {
                num += this.CalculateRowHeight( this.cachedEntries[i] );
            }
            return num;
        }

        private TableDef<T> def;

        private Func<IEnumerable<T>> entryGetter;
        ...
        private ColumnDef<T> sortByColumn;

        private bool sortDescending;

        private Vector2 cachedSize;

        private List<T> cachedEntries = new List<T>();
